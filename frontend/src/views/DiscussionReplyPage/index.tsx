...
export default function DiscussionReplyPage(props: propsType): React.ReactElement {
    ...
    const postReply  = async (parent_id:string , index:number) =>{
        if (userData){
            const uid = userData?.uid;
            const publisher = userData?.name;
            const reply : DiscussionTopicReply = {
                parent_id : parent_id ,
                topic_id : params.discussionTopicId|| "",
                publisher_avatar : "" ,
                uid:uid,
                publisher:publisher,
                content:"",
            ...
            if (!categorizedReplies[Number(parent_id)]) {
                categorizedReplies[Number(parent_id)] = [];
            }
            categorizedReplies[Number(parent_id)].push(reply);
                            ...
                            <div key={index} className="discussionTopicReply">
                                ...
                                { showReplyAreaList[index] === true &&  <div className="discussionReplyArea">
                                    ...
                                    <textarea
                                        ...
                                        rows={1}
                                    />
                                    <IoSend className="sendIcon" onClick={() => postReply(data.id||"" , index)}/>
                                </div>}
